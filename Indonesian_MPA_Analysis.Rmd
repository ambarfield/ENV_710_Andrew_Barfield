...
histogram_1 <- ggplot(settlement_data, aes(x = Food_Security_Index,
  ...
  labs(x = "Food Security Index", 
       ...
       title = "Food Security Index Distributions within Six MPAs in Papua, Indonesia") +
...
fs_pa_scatterplot <- ggplot(settlement_data, aes(x = Food_Security_Index,
  ...
  labs(x = "Food Security Index",
       ...
       color = "MPA Name") +
  ggtitle("Relationship Between Household Food Security 
...
# Descriptive Statistics (Andrew)
```{r}
# Summary Table
indonesia_summary <- settlement_data %>% 
group_by(MPAName) %>%
  summarise(count = n(),
            mean_food_security = mean(Food_Security_Index, na.rm = TRUE),
            mean_place_attachment = mean(Place_Attachment_Index,
                                               na.rm = TRUE),
            median_food_security = median(Food_Security_Index, na.rm = TRUE),
            median_place_attachment = median(Place_Attachment_Index, na.rm = TRUE),
            variance_food_security = var(Food_Security_Index, na.rm = TRUE),
            variance_place_attachment = var(Place_Attachment_Index, na.rm = TRUE),
            sd_food_security = sd(Food_Security_Index, na.rm = TRUE),
            sd_place_attachment = sd(Place_Attachment_Index, na.rm = TRUE)) %>%
  ungroup()

# Rename columns
colnames(indonesia_summary) <- c("MPA Name", "Number of Observations", "Mean Food Security",
                                 "Mean Place Attachment", "Median Food Security",
                                 "Median Place Attachment", "Variance Food Security",
                                 "Variance Place Attachment", "SD Food Security",
                                 "SD Place Attachment")
...
indonesia_gt <- indonesia_summary %>% gt() %>% 
  fmt_number(columns = c(`Mean Food Security`, `Mean Place Attachment`, `Median Food Security`,
                         `Median Place Attachment`, `Variance Food Security`,
                         `Variance Place Attachment`, `SD Food Security`,
                         `SD Place Attachment`),
             decimals = 2)
